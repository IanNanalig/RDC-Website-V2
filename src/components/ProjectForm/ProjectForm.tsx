...
type Schema = { title: string; steps: Step[] };
...
export default function ProjectForm({ initial, onSave, onCancel }: Props) {
  ...
  const schemaTyped = schema as unknown as Schema;
  const steps = schemaTyped.steps;

  const onSubmit = async (data: Record<string, unknown>) => {
    setLoading(true);
    try {
      // extract files if any
      const files = (data as Record<string, unknown>)["attachments"] ?? null;
      ...
      steps.forEach((s) =>
        s.fields.forEach((f) => {
          if (f.type === "array" && typeof payload[f.name] === "string") {
            const val = payload[f.name] as string;
            payload[f.name] = val
              .split(",")
              .map((x) => x.trim())
              .filter(Boolean);
    ...
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 16 }}>
      ...
      <div style={{ border: "1px solid #eee", padding: 12 }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          {steps[stepIndex].fields.map((field: Field) => (
            <FieldRenderer
              key={field.name}
              field={field}
              register={register}
