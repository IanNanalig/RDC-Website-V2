...
const Dashboard: React.FC = () => {
  ...
  const [activityHasPrevious, setActivityHasPrevious] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchActivity = async () => {
      try {
        const data = await api.get(`agency/activity/?include_meta=true&limit=${activityLimit}&offset=${activityOffset}`);
        if (!mounted) return;
  ...
  const activityLimitNumber = Math.max(1, Number(activityLimit) || 10);
  const activityStart = activityCount > 0 ? activityOffset + 1 : 0;
      ...
      <div className="portal-card mt-4">
        ...
        {activityLoading ? (
          ...
          <div className="portal-card-body">
            ...
            <div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-3 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
              <span>
                Showing {activityStart}-{activityEnd} of {activityCount}
              </span>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setActivityOffset((current) => Math.max(0, current - activityLimitNumber))}
                  disabled={!activityHasPrevious}
                  className="rounded border px-3 py-1 font-medium text-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Previous
                </button>
                <button
                  type="button"
                  onClick={() => setActivityOffset((current) => current + activityLimitNumber)}
