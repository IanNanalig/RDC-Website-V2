...
type ProfileForm = {
  ...
  priorityAnalysisFacts: PriorityAnalysisFacts;
  remarks: string;
  focalPerson: string;
...
const initialForm: ProfileForm = {
  ...
  provincialBreakdown: "",
  projectCostRows: [{ source: "", y2022Prior: "", y2023: "", y2024: "", y2025: "", y2026: "", y2027: "", y2028: "", y2029: "", continuingYears: "", yearValues: {}, overall: "" }],
  pipBudgetRows: [{ year: "", osbps: "", nep: "", gaa: "" }],
  provincialRows: [{ province: "", y2022Prior: "", y2023: "", y2024: "", y2025: "", y2026: "", y2027: "", y2028: "", y2029: "", continuingYears: "", yearValues: {}, overall: "" }],
  levelOfApproval: "",
  mainPdpChapter: "",
  mainPdpOutcome: "",
  mainPdpSubOutcome: "",
...
const fieldLabels: Record<keyof ProfileForm, string> = {
  ...
  priorityAnalysisFacts: "Priority Analysis Facts",
  remarks: "Remarks",
...
type FundingColumn = {
  key: string;
  label: string;
  fixedKey?: "y2022Prior" | "y2023" | "y2024" | "y2025" | "y2026" | "y2027" | "y2028" | "y2029";
};

const fixedFundingKeyByYear: Record<string, FundingColumn["fixedKey"]> = {
  "2022_prior": "y2022Prior",
  "2023": "y2023",
  "2024": "y2024",
  "2025": "y2025",
  "2026": "y2026",
...
const getFundingValue = (row: ProjectCostRow | ProvincialRow, column: FundingColumn) =>
  column.fixedKey ? row[column.fixedKey] : row.yearValues?.[column.key] || "";

const sumFundingValues = (row: ProjectCostRow | ProvincialRow) =>
  projectCostYearKeys.reduce((sum, key) => sum + toNumber(row[key]), 0) +
  Object.values(row.yearValues || {}).reduce((sum, value) => sum + toNumber(value), 0);

const formatFundingEntries = (row: ProjectCostRow | ProvincialRow) => {
  const fixedEntries = [
    ["2022&prior", row.y2022Prior],
    ["2023", row.y2023],
    ["2024", row.y2024],
    ["2025", row.y2025],
    ["2026", row.y2026],
  ...
  const dynamicEntries = Object.entries(row.yearValues || {}).sort(([a], [b]) => Number(a) - Number(b));
  const legacyEntries = row.continuingYears ? [["continuing", row.continuingYears]] : [];
  return [...fixedEntries, ...dynamicEntries, ...legacyEntries].map(([label, value]) => `${label}=${value}`);
};

const formatProjectCostRowSummary = (row: ProjectCostRow) =>
  `${row.source}: ${[...formatFundingEntries(row), `total=${row.overall}`].join(", ")}`;
...
const normalizeProjectCostRow = (row: Record<string, unknown>): ProjectCostRow => {
  const normalized: ProjectCostRow = {
    ...
    yearValues: row.yearValues && typeof row.yearValues === "object" && !Array.isArray(row.yearValues)
      ? Object.fromEntries(Object.entries(row.yearValues).map(([key, value]) => [key, String(value || "")]))
      : {},
    overall: String(row.overall || row.total || ""),
  };
  const computedOverall = sumFundingValues(normalized);
  ...
  <div>
    ...
    <div className={`columns-1 xl:columns-2 gap-6 border rounded p-2 ${error ? (error.startsWith("Original:") ? "border-amber-500 bg-amber-50" : "border-red-500 bg-red-50") : ""}`}>
      {options.map((option) => {
        ...
        return (
          <label key={option} className="mb-1 text-sm flex items-start gap-2 min-w-0 break-inside-avoid">
            <input
...
const ProjectSubmission: React.FC = () => {
  ...
  const setPriorityFact = <K extends keyof PriorityAnalysisFacts>(key: K, value: PriorityAnalysisFacts[K]) => {
    setForm((prev) => ({
      ...prev,
      priorityAnalysisFacts: {
        ...prev.priorityAnalysisFacts,
        [key]: value,
      },
    }));
  };
  ...
  const updateProjectCostRow = (index: number, key: keyof ProjectCostRow, value: string) => {
    setForm((prev) => ({
      ...
      projectCostRows: prev.projectCostRows.map((row, i) => {
        ...
        const computedOverall = sumFundingValues(nextRow);
  ...
  const updateProjectCostYearValue = (index: number, column: FundingColumn, value: string) => {
    ...
    setForm((prev) => ({
      ...
      projectCostRows: prev.projectCostRows.map((row, i) => {
        ...
        const nextRow = {
          ...row,
          yearValues: { ...(row.yearValues || {}), [column.key]: value },
        };
        return { ...nextRow, overall: fmtNumber(sumFundingValues(nextRow)) };
      }),
    }));
    setStepErrors((prev) => {
      const next = { ...prev };
  ...
  const hasProjectCostRow = form.projectCostRows.some((row) =>
    [row.source, row.y2022Prior, row.y2023, row.y2024, row.y2025, row.y2026, row.y2027, row.y2028, row.y2029, row.continuingYears, row.overall, ...Object.values(row.yearValues || {})].some((v) => String(v).trim()),
  );
  ...
  const hasProvincialRow = form.provincialRows.some((row) =>
    [row.province, row.y2022Prior, row.y2023, row.y2024, row.y2025, row.y2026, row.y2027, row.y2028, row.y2029, row.continuingYears, row.overall, ...Object.values(row.yearValues || {})].some((v) => String(v).trim()),
  );
  const requiresProvincialBreakdown = form.provincialBreakdownOption !== "No available breakdown information (Kindly skip)";
  const activeFundingYearColumns = useMemo(() => {
    if (!fundingRange) return [];
    const columns: FundingColumn[] = [];
    if (fundingRange.minYear <= 2022 && fundingRange.maxYear >= 2022) {
      columns.push({ key: "2022_prior", label: "2022 & Prior", fixedKey: "y2022Prior" });
    }
    const firstYear = fundingRange.minYear <= 2022 ? 2023 : fundingRange.minYear;
    for (let year = firstYear; year <= fundingRange.maxYear; year += 1) {
      const key = String(year);
      columns.push({ key, label: key, fixedKey: fixedFundingKeyByYear[key] });
    }
    return columns;
  }, [fundingRange]);
  const pipTrackerYearOptions = useMemo(() => {
    if (!fundingRange) return [];
    const firstYear = Math.max(2023, fundingRange.minYear);
    const years: string[] = [];
    for (let year = firstYear; year <= fundingRange.maxYear; year += 1) {
  ...
  const projectCostTotals = useMemo<Record<string, number>>(() => {
    ...
    form.projectCostRows.forEach((row) => {
      activeFundingYearColumns.forEach((column) => {
        totals[column.key] = (totals[column.key] || 0) + toNumber(getFundingValue(row, column));
      });
      totals.overall += sumFundingValues(row);
    });
    return totals;
  }, [activeFundingYearColumns, form.projectCostRows]);
  ...
  const provincialTotals = useMemo<Record<string, number>>(() => {
    const totals: Record<string, number> = {
      y2022Prior: 0,
      y2023: 0,
      y2024: 0,
    ...
    return totals;
  }, [activeFundingYearColumns, form.provincialRows]);
  ...
  useEffect(() => {
    ...
    form.projectCostRows.forEach((row) => {
      ...
      if (!source) return;
      const amount = sumFundingValues(row);
  ...
  useEffect(() => {
    ...
    setForm((prev) => {
      ...
      const nextProjectRows = prev.projectCostRows.map((row) => {
        ...
        if (next.continuingYears) {
          next.continuingYears = "";
          changed = true;
        }
        const nextYearValues = Object.fromEntries(
          Object.entries(next.yearValues || {}).filter(([key, value]) => {
            const keep = activeDynamicYearKeys.has(key);
            if (!keep && value) changed = true;
            return keep;
          }),
        );
        if (Object.keys(nextYearValues).length !== Object.keys(next.yearValues || {}).length) {
          next.yearValues = nextYearValues;
        }
        const computedOverall = sumFundingValues(next);
              ...
              <div className="space-y-3">
                    ...
                    onClick={() =>
                      setForm((prev) => ({
                        ...
                        projectCostRows: [
                          ...
                          { source: "", y2022Prior: "", y2023: "", y2024: "", y2025: "", y2026: "", y2027: "", y2028: "", y2029: "", continuingYears: "", yearValues: {}, overall: "" },
                        ],
                      }))
                    }
                  ...
                  {form.projectCostRows.map((row, index) => (
                    <div key={`cost-${index}`} className="rounded-lg border border-slate-200 bg-white p-3 space-y-3">
                      ...
                      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-6">
                        ...
                        {activeFundingYearColumns.map((column) => (
                          <label key={column.key} className="block">
                            <span className="text-xs text-slate-600">{column.label}</span>
                            <input
                        ...
                        {activeFundingYearColumns.length === 0 && (
                          <p className="sm:col-span-2 lg:col-span-3 2xl:col-span-5 text-xs text-slate-500">
                            Set Start Year and Completion Year to show funding columns.
                          </p>
                        )}
                ...
                <div className="rounded-lg border bg-slate-50 p-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 2xl:grid-cols-10 gap-2 text-xs">
                    {activeFundingYearColumns.map((column) => (
                      <div key={column.key} className="rounded border bg-white px-2 py-1"><span className="block text-slate-500">{column.label}</span><strong>{fmtNumber(projectCostTotals[column.key]) || "0"}</strong></div>
                    ))}
                ...
                {requiresProvincialBreakdown && (
                  <>
                      ...
                      <button
                        ...
                        onClick={() =>
                          setForm((prev) => ({
                            ...
                            provincialRows: [
                              ...prev.provincialRows,
                              { province: "", y2022Prior: "", y2023: "", y2024: "", y2025: "", y2026: "", y2027: "", y2028: "", y2029: "", continuingYears: "", yearValues: {}, overall: "" },
                            ],
                          }))
                        }
                        className="px-2 py-1 text-sm border rounded"
                    ...
                    <div className="rounded-lg border bg-slate-50 p-3">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 2xl:grid-cols-10 gap-2 text-xs">
                        {activeFundingYearColumns.map((column) => (
                          <div key={column.key} className="rounded border bg-white px-2 py-1"><span className="block text-slate-500">{column.label}</span><strong>{fmtNumber(provincialTotals[column.key]) || "0"}</strong></div>
                        ))}
              ...
              {!isValidator && (
                <div className="rounded-lg border border-emerald-200 bg-emerald-50/40 p-4 space-y-4">
                  ...
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <Field label="Estimated Beneficiary Count" value={form.priorityAnalysisFacts.beneficiaryCount} onChange={(v) => setPriorityFact("beneficiaryCount", v)} />
                  </div>
                  <Area label="Readiness Evidence Notes" value={form.priorityAnalysisFacts.readinessNotes} onChange={(v) => setPriorityFact("readinessNotes", v)} rows={2} />
                </div>
              )}
